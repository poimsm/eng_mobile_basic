...
//       showQuestionReplayBtn: showQuestionReplayBtn ?? this.showQuestionReplayBtn,      
//       runRoutineWhenTimeCompleted: runRoutineWhenTimeCompleted ?? this.runRoutineWhenTimeCompleted,
//     );
//   }
// }
...
//       if (state.seconds == maxSpeakingTime && state.runRoutineWhenTimeCompleted) {
//         await record.stop();
//         bool shouldHaveSavedAudio = state.isRecording ? true : false;
//         state = state.copyWith(hasAudioSaved: false, isRecording: false, seconds: 0);
//         stopMic();
//         await sleep(500);
...
//       int wordId = state.word!.id;
//       final examples = state.question!.examples;

//       final ex = examples.firstWhere((x) => x.wordId == wordId);
//       state = state.copyWith(example: ex);      
//       currentExamplePlay++;
//       _playExample(ex, currentExamplePlay, state.word!.word);
...
//   void _playExample(QuestionExample ex, int index, String word) async {
//     for (var i = 0; i < ex.example.length; i++) {
//       if (index != currentExamplePlay) break;

//       state = state.copyWith(
//           exampleArry: _buildExample(ex.example[i].value, state.word!.word));
